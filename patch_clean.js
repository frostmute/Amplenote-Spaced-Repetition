...
const newExtract = `  _extractFlashcardsFromMarkdown: function(content) {
    ...
    let registry = {};
    const registryRegex = /## ⚙️ Spaced Repetition Data[\\s\\S]*?\\n\`\`\`json\\n([\\s\\S]*?)\\n\`\`\`/;
...
// Cut out exact chunks by splitting the entire file logically
const p1 = data.split('  _extractFlashcardsFromMarkdown: function(content) {')[0];

const afterExt = data.split('  _extractFlashcardsFromMarkdown: function(content) {')[1];
const p2 =
  '  _linesToMarkdown: function(lines) {' +
  afterExt
    .split('  _linesToMarkdown: function(lines) {')[1]
    .split('  _saveCardToNote: async function(app, card) {')[0];

const afterSave = data.split('  _saveCardToNote: async function(app, card) {')[1];
const p3 = '  // --- UI Templates ---' + afterSave.split('  // --- UI Templates ---')[1];

let finalData = p1 + newExtract + '\\n\\n' + newUpdate + '\\n\\n' + p2 + newSave + '\\n\\n' + p3;

// Fix Dashboard Update Gap
finalData = finalData.replace(
  'i++;\\n    }\\n    \\n    await app.alert',
  'i++;\\n    }\\n    if (completed > 0) { await this._updateDashboard(app, this._currentReviewSession); }\\n    await app.alert'
);

// Fix Dashboard Stats gap
finalData = finalData.replace(
  '| 🟣 Easy | ${stats.easy} | ${easyPct}% |\\n\\n<!--STATS:${JSON.stringify(stats)}-->`;',
  '| 🟣 Easy | ${stats.easy} | ${easyPct}% |\\n<!--STATS:${JSON.stringify(stats)}-->`;'
);

// Fix Session Card Array Bug
finalData = finalData.replace(
...
console.log('Rewritten correctly via exact splits.');
