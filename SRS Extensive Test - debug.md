---
title: SRS Extensive Test - debug
uuid: e2c8974a-69de-11f1-aa55-6918fcc5bf06
version: 217
created: '2026-06-16T18:55:43-05:00'
updated: '2026-06-20T01:55:11-05:00'
tags:
  - learning
  - flashcards
---

---

### JavaScript Basics

|                                                              |                                         |     |
| ------------------------------------------------------------ | --------------------------------------- | --- |
| Question                                                     | Answer                                  |     |
| What is the global object in a browser environment?          | `window`                                |     |
| What is the global object in a Node.js environment?          | `global`                                |     |
| How do you declare a block-scoped variable in modern JS?     | `let` or `const`                        |     |
| What does `typeof null` return in JS?                        | `"object"`                              |     |
| How do you deep clone an object without libraries (modern)?  | `structuredClone(obj)`                  |     |
| Which JS array method adds an element to the beginning?      | `unshift()`                             |     |
| Which JS array method removes an element from the beginning? | `shift()`                               |     |
| What does the `===` operator do?                             | Strict equality (checks value and type) |     |
| What does `NaN` stand for?                                   | Not-a-Number                            |     |
| How do you check if a property exists on an object?          | `hasOwnProperty()` or `in` operator     |     |

\

\

### Shifted Columns / Edge Cases (Testing Parser)

\

|                          |                           |     |
| ------------------------ | ------------------------- | --- |
| Question                 | Answer                    |     |
| What does DOM stand for? | Document Object Model     |     |
|                          | What does CSS stand for?  |     |
|                          | What does HTML stand for? |     |

\

\

### Python Fundamentals

\

|                                                     |                              |     |
| --------------------------------------------------- | ---------------------------- | --- |
| Question                                            | Answer                       |     |
| How do you define a function in Python?             | `def my_func():`             |     |
| What data structure uses key-value pairs in Python? | Dictionary (`dict`)          |     |
| How do you initialize an empty list in Python?      | `my_list = \[\]`             |     |
| How do you read a file completely into a string?    | `file.read()`                |     |
| What keyword is used to handle exceptions?          | `try` / `except`             |     |
| What is the default string encoding in Python 3?    | UTF-8                        |     |
| How do you get the length of a list in Python?      | `len(my_list)`               |     |
| What does `__init__` do in a Python class?          | It is the constructor method |     |

\

\

### CSS and Layout

\

|                                                        |                                                     |     |
| ------------------------------------------------------ | --------------------------------------------------- | --- |
| Question                                               | Answer                                              |     |
| What property changes the background color?            | `background-color`                                  |     |
| How do you center an element using Flexbox?            | `justify-content: center; align-items: center;`     |     |
| What does the `z-index` property do?                   | Controls the stacking order of overlapping elements |     |
| What is the default value of the `position` property?  | `static`                                            |     |
| What unit is relative to the root element's font size? | `rem`                                               |     |
| How do you make a font italic in CSS?                  | `font-style: italic;`                               |     |

\

\

### Amplenote Specific

\

|                                        |                                      |     |
| -------------------------------------- | ------------------------------------ | --- |
| Question                               | Answer                               |     |
| What does Jots stand for in Amplenote? | Daily notes/scratchpad               |     |
|                                        | How do you link to another note?     |     |
|                                        | How do you open the command palette? |     |
