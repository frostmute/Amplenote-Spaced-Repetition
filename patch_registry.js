...
const updateNew = `  _updateFlashcardInLines: function(lines, card) {
    let headerLineIdx = -1;
...
const saveNew = `  _saveCardToNote: async function(app, card) {
...
// Replace functions exactly
plugin = plugin.replace(
  /_extractFlashcardsFromMarkdown: function\(content\) \{[\s\S]*?return \{ flashcards, lines \};\n  \},/,
  extractNew
);
plugin = plugin.replace(
  /_updateFlashcardInLines: function\(lines, card\) \{[\s\S]*?return true;\n  \},/,
  updateNew
);
plugin = plugin.replace(
  /_saveCardToNote: async function\(app, card\) \{[\s\S]*?await note\.replaceContent\(newContent\);\n  \},/,
  saveNew
);
...
console.log('Rewritten correctly via Node JS string replacement.');
